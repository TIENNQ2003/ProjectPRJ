 ...
 * @author nqtie
 */
@WebServlet(urlPatterns = {"/CartURL"})
public class CartController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (service.equals("deleteCart")) {
            ...
            dao.deleteCart(user.getUserID(), proid);
            response.sendRedirect("ShoppingCartURL");
        }
        if (service.equals("updateCart")) {
            ...
            if (quantity > pro.getUnitinStock()) {
                ...
                request.getRequestDispatcher("ShoppingCartURL").forward(request, response);
            } else {
                String dateInsert = dao.getDate("select getdate()");
                Cart cart = new Cart(user.getUserID(), proid, quantity, dateInsert);
                dao.updateCart(cart);
